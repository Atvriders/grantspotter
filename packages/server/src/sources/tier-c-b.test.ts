import type { OpportunityClass, SourceModule } from '@grantspotter/core';
...
/** The fixed corpus clock, so a status assertion below reads against the same `now` the profiler does. */
const NOW_ISO = '2026-08-02T00:00:00.000Z';

function ctxFor(
  sourceId: string,
  funderId: string,
  klass: OpportunityClass = 'adjacent_stem',
): NormalizeContext {
  return {
    sourceId,
    funderId,
...
/**
 * Every shape below is lifted from a committed capture or one of its siblings — none is invented to
 * flatter the parser. The four sentences the reader exists for are asserted against their own real
 * fixtures in the `(REAL fixture)` blocks; this block is the grammar underneath them, plus the
 * decoys that share those pages.
 */
...
describe('ieee-mtts (REAL fixture)', () => {
  ...
  /**
   * THE DEADLINE IS READ AS A MONTH-DAY AND NOT RESOLVED, because IEEE never writes a year next to
   * it. Verbatim (capture line 179 flattened, raw HTML line 619):
   *
   *   "IMPORTANT: All requests for MTT chapter funding must be received by October 1 or the
   *    chapter may be asked to make its application in the following year."
   *
   * "must be received by", not "due" — the wording that already cost this source every record it
   * had. One date, so it is a close and not an open. And "the following year" is MTT-S telling us
   * this repeats annually, which is a RECURRENCE (`RECUR annual_window window=10-01..10-01`), not
   * a dated window: the dated-window channel would have to name a year, and the only years on this
   * capture are event titles and "© Copyright 2026 IEEE".
   */
  it('reads 10-01 out of "must be received by October 1" and states no year', () => {
    const window = parseProseWindow(raws[0].rawFields.deadline!);
    expect(window).toEqual({ closesOn: '10-01', yearUnstated: true });
    expect(raws[0].rawFields.opensAt).toBeUndefined();
    expect(raws[0].rawFields.closesAt).toBeUndefined();
  });

  /**
   * THE DECOY THAT SHARES THE PAGE. "Minimum of ten (10) members; five (5) members for Student
   * Branch Chapters" is a membership threshold, and the numerals in it are the shape a date parser
   * is most likely to mistake for a day. Nothing in it is adjacent to a month name, so nothing in
   * it is a date — asserted here rather than assumed, because the cost of getting it wrong is a
   * chapter told its funding closes on the 5th of some month.
   */
  it('never reads a date out of the spelled-out, parenthesised membership numbers', () => {
    expect(parseProseWindow(raws[0].rawFields.requirements!)).toBeUndefined();
    expect(parseProseWindow('The event must be at least four (4) hours in length')).toBeUndefined();
  });

  it('stays unknown rather than open, because nothing here can date October 1', () => {
...
describe('ieee-student-branch-rebate (REAL fixture)', () => {
  ...
  /**
   * DAY BEFORE MONTH, AND NO YEAR. Verbatim: "Student Branch Annual Plans are due 15 March."
   *
   * IEEE writes its dates the international way round, which is the second of the two orders
   * `parseProseWindow` scans for. The year is unstated, and the two years that DO appear on the
   * page are traps: the post's own byline, "July, 27 2022, Robert Sacks", and "© Copyright 2026
   * IEEE". One is when the notice was written and the other is the site's copyright; neither is
   * when a Branch Annual Plan is due. So the month-day is read and no date is published.
   */
  it('reads 03-15 from the day-before-month form and states no year', () => {
    const window = parseProseWindow(raws[0].rawFields.deadline!);
    expect(window).toEqual({ closesOn: '03-15', yearUnstated: true });
    expect(raws[0].rawFields.opensAt).toBeUndefined();
    expect(raws[0].rawFields.closesAt).toBeUndefined();
  });

  it('has no year on the page that is the deadline"s year', () => {
    // The byline year and the copyright year, and nothing else.
    expect([...new Set(raws[0].rawText.match(/\b(?:19|20)\d{2}\b/g) ?? [])].sort()).toEqual([
      '2022',
      '2026',
    ]);
    expect(raws[0].rawFields.deadline).not.toMatch(/\b(?:19|20)\d{2}\b/);
  });

  it('stays unknown rather than open, because nothing here can date 15 March', () => {
  ...
  /**
   * THE OTHER HALF OF THE REFUSAL: the parser is not broken, it is being honest. Hand it the same
   * sentence with a year in it and it resolves immediately — which is what will happen by itself
   * the day IEEE prints one, with no code change.
   */
  it('resolves the same sentence the moment a year appears in it', () => {
    expect(parseProseWindow('Student Branch Annual Plans are due 15 March 2027.')).toEqual({
      closesOn: '03-15',
      closesAt: '2027-03-15',
      yearUnstated: false,
    });
