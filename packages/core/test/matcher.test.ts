...
import { isDoNotPublish, loadCorpus, PROFILE_NOW_ISO, PROFILES } from '../../../scripts/profile-corpus.js';
import {
 ...
 * blind spot: the CWops Scholarship's trade/art/professional-school disjunct (fixed just above,
 * commit ec64293) had no profile that could ever exercise it, and 76 of the corpus's 197
 * publishable programs are org-facing (club/school/university) and were never matched against
 * anything — which is exactly the population the `do_not_publish` bug (37 ARRL club-grant past
 * recipients, 424 ARDC award rows, 45 USAspending and 38 NSF historical awards, all publishable)
 * landed on. `PROFILES` in scripts/profile-corpus.ts now also carries `cert-trade` (a licensed
