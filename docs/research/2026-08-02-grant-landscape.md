...
| **ARRL Foundation** | Scholarship Program — **111 catalog entries / "170+" awards** | Individual FCC-licensed students in higher ed; some entries open to foreign licensees | **$500–$25,000**. ⚠️ "2024: 135 awards, >$715,000" is **unverified** — appears zero times in any captured ARRL scholarship page; see §6.6 | Annual: opens ~**Oct 30**, closes ~**Dec 30** (dates only — no capture states a closing time, a timezone, or a prior date it "moved from"; "12:00 PM EST" and "Moved from Jan 31" are **unverified**, see §6.6) | http://www.arrl.org/scholarship-descriptions | **Tier C — highest-yield scrape in the entire space.** 4 accordions `div.tabArea.f-widget.f-accordion`, `ul.accordion > li`, 114 li − 3 stubs = **111 real entries** | Verified (2, with full DOM spec) |
...
| **Yaesu USA** | System Fusion DR-2X Repeater Program | ⚠️ Eligibility text **unverified** — "North America" / "individuals" / "organizations" appear zero times in the captured page; see §6.6. Collegiate clubs aren't excluded by anything the page states, but nothing on it names them either | Purchase programme, not a grant (the word "grant" never appears): the page states two prices, **$1,450.00** and **$1,860.00**, without saying what distinguishes them — the "$1,860 = with LAN-01A" pairing is ⚠️ **unverified**, see §6.6 | Ad-hoc windows ~2–4/yr. Confirmed: closes **Aug 31, 2026** (page text). No open date is published beyond the month-granular PDF filename `DR-2X_Jun-thru-Aug_2026`; "Jun 3" is ⚠️ **unverified**, see §6.6. Any on-air sustainment term lives only in the linked fillable PDF (never downloaded) — **unknown**, not confirmed | https://systemfusion.yaesu.com/ | **Tier C/D — PDF-only.** Window dates live in a dated fillable PDF title under `/wp-content/uploads/{YYYY}/{MM}/`; poll the landing page for the link | Verified |
...
**6. Some of the "Verified" cells above did not survive contact with the actual captured bytes.** This pass was written from live browsing before this project had committed page captures to check claims against. Task 9's later byte-level audit (2026-08, `.superpowers/sdd/2026-08-02-grantspotter-plan-4-writing/task-9-report.md`) grepped the committed HTML case-insensitively for the specific claims in rows 17 and 36 above and found each of the following absent, **zero times**: on the 145,639-byte Yaesu DR-2X capture — `12 month`, `12-month`, `twelve`, `on the air`, `on-air`, `obligat`, `remain` (the "repeater must remain on the air for 12 months" claim), `lan-01a`, `network module` (the "$1,860 with the LAN-01A" pairing — the page states two prices and never says what distinguishes them), `north america`, `discount`, `grant` in Yaesu's own voice, and `june 3` (the claimed opening date); and, on the ARRL scholarship captures — `12:00`, `noon`, `eastern`, `january 31`, `715`, and `135 award`/`135 scholar` (the close time, the "moved from" claim, and the 2024 award total). The 12-month figure traced to a hand-written fixture; the June 3 date and the ARRL close time traced to `pathological.html`, a synthetic fixture built to exercise the parser, not a capture of a live page. Where a funder page states an obligation but the crawler deliberately never downloads a linked PDF (Yaesu's application form), the honest status is **unknown**, not false — the row edits above say that rather than silently dropping the claim. Lesson for future research passes on this project: a plausible-sounding specific (a time zone, a SKU-looking accessory code, a date) is exactly the kind of detail that reads as researched and is the one most worth checking against committed bytes before it is repeated as fact.